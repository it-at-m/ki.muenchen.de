...
const startGame = () => {
  ...
  message.value = `Level ${currentLevel.value}: Verwende die PFEILTASTEN zum Bewegen und LEERTASTE zum Springen!`;
...
const nextLevel = () => {
  ...
  if (currentLevel.value <= levels.length) {
    loadLevel(currentLevel.value);
    message.value = `Level ${currentLevel.value}: Richtig Schwierig!`;
...
const updateGame = () => {
  ...
  if (coins.value.every((coin) => coin.collected)) {
    if (currentLevel.value < levels.length) {
      ...
      message.value = `Level ${currentLevel.value} geschafft! +500 Bonus Punkte`;
      nextLevel();
    } else {
  ...
  <div class="kicc-escape">
    ...
    <div v-else>
      ...
      <p>
        ...
        weiche den Gegnern aus! Nur dann wirst du zurück auf die Startseite
        finden!
      </p>
      ...
      >
        <div class="game-hud">
          ...
          <div>Leben: {{ "❤️".repeat(lives) }}</div>
          <div>
            Disketten: {{ coins.filter((c) => c.collected).length }}/{{
              coins.length
            }}
          </div>
        </div>
        ...
        <div class="game-controls">
          ...
          >
            {{ gameWon ? "Nochmal spielen" : "Spiel beenden" }}
          </button>
        </div>
      </div>
    </div>
